...
import { magnitude } from "../../math/vec"
import { TransformRotate } from "../physics/transformRotate"
import { TileSprite } from "../../shapes/tile-sprite"
...
export const createLineColor = (manager, x, y, tox, toy, width, color, stroke) => {
    //TODOFIX
    let entity = manager.create()
    let dx = tox - x 
    let dy = toy - y 
    let half = {x:(x+tox)/2, y:(y+toy)/2}
    let angle = Math.atan2(dy,dx)

    manager.asign(new Transform([half.x - Math.cos(angle)*dx/2, half.y - Math.sin(angle)*dy/2]), entity)
    length = magnitude(dx,dy)
    
    manager.asign(new ShapeBox(length, width), entity)
    manager.asign(new Renderer(color, stroke), entity)
    manager.asign(new TransformRotate(angle), entity)
    return entity
}



export const createCircleColor = (manager, x, y, radius, color, stroke) => {
    let entity = manager.create()
    manager.asign(new Transform([x, y]), entity)
...
export const createTiledSprite = (manager, x, y, element, sx, sy, tx, ty) => {
    let entity = manager.create()
    manager.asign(new Transform([x, y]), entity)
    if (sx && sy) {
        manager.asign(new ShapeBox(sx, sy), entity)
    ...
    let img = ImageHash.get(element || planeBase64)
    manager.asign(new TileSprite(img, tx, ty), entity)
    manager.asign(new Renderer(), entity)
    return entity
}
